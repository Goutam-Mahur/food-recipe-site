...
const Details = () => {
  ...
  return (
    <div className="container mx-auto py-10 grid grid-cols-1 lg:grid-cols-2 gap-10">
      <div className="flex justify-center mt-1 items-start overflow-hidden rounded-xl">
        <img
          src={recipeDetails?.recipe?.image_url}
          alt="image of recipe"
          className="w-[80%] lg:w-[90%] max-h-52 lg:h-1/2 lg:max-h-none m-2 lg:m-0 object-cover block hover:scale-105 hover:rounded-lg duration-300 rounded-xl"
          onError={(e) => {
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          ...
          <div className="text-2xl mb-2 mt-4 underline font-semibold text-black">
            INGREDIENTS
          </div>
          ...
          <ul className="flex flex-col gap-3 p-2">
            {recipeDetails?.recipe?.ingredients.map((Ingredients, index) => (
              <li className="list-disc" key={index}>
                <span className="text-xl my-3 ml-1 font-medium text-black">
                  {Ingredients?.quantity === "" ? "1" : Ingredients?.quantity}{" "}
                  {Ingredients.unit}{" "}
                </span>
                <span className="text-xl my-3 font-medium text-black">
